    ...
    void HandleMusicFade()
    {
        ...
        switch (musicState)
        {
            case MusicState.FadingIn:
                ...
                GetComponent<AudioSource>().volume = Mathf.Min(GetComponent<AudioSource>().volume + fadeFactor * Time.deltaTime, musicFullVolume);
               
                // Check if we reached full volume and switch state to FullVolume
                if (GetComponent<AudioSource>().volume >= musicFullVolume)
            ...
            case MusicState.FadingOut:
                ...
                GetComponent<AudioSource>().volume = Mathf.Max(GetComponent<AudioSource>().volume - fadeFactor * Time.deltaTime, 0);
